...
func TestParseVersionDirective_UntilQuote(t *testing.T) {
	...
	parseVersionDirective := func(c *Cursor) (VersionDirective, error) {
		...
		// 6️⃣ Consomme tout jusqu'à QUOTE ou NEWLINE
		valueTokens, err := UntilAny(
			Consume(syntax.QUOTE),   // stop sur quote fermante
			Consume(syntax.NEWLINE), // stop sur fin de ligne (erreur si pas quote)
		).Parse(c)

		if err != nil {
			return VersionDirective{}, fmt.Errorf("error consuming version value: %v", err)
		}

		// 7️⃣ Vérifie que le stop était bien QUOTE, pas NEWLINE
		if c.tokens[c.pos].Kind() == syntax.NEWLINE {
			return VersionDirective{}, fmt.Errorf("expected closing quote before end of line")
		}

		// 8️⃣ Consomme la quote fermante
		_, err = Consume(syntax.QUOTE).Parse(c)
		if err != nil {
